...
function fix_dns_commas() {
    dns_line=$(grep -i "option domain-name-servers" /etc/dhcp/dhcpd.conf)

    # Check if there are multiple DNS servers without commas
    dns_count=$(grep -o "option domain-name-servers" /etc/dhcp/dhcpd.conf | wc -l)

    if [ "$dns_count" -gt 1 ]; then
        # Add commas between DNS servers
        sed -i 's/option domain-name-servers \(.*\);/option domain-name-servers \1;/g' /etc/dhcp/dhcpd.conf
    else
    fi
}
...
function remove_iptables_rules() {
    ...
    read -p "Enter the protocol (tcp/udp): " target_protocol

    # Remove post-up rule
    sed -i "/post-up iptables -t nat -A PREROUTING -i vmbr0 -p $target_protocol --dport $target_port -j DNAT --to $target_ip:$target_port/d" /etc/network/interfaces

    # Remove post-down rule
    sed -i "/post-down iptables -t nat -D PREROUTING -i vmbr0 -p $target_protocol --dport $target_port -j DNAT --to $target_ip:$target_port/d" /etc/network/interfaces

    echo "iptables rules for IP $target_ip, port $target_port, and protocol $target_protocol have been removed."
}
...
function select_task() {
    echo -e "\nSelect a task:"
    PS3="Enter the task number: "
    options=("Remove enterprise dependencies" "Configure IP forwarding" "Configure DHCP server" "Configure static IP" "Assign ports" "Remove Rules (IPTables)" "Exit")
